registerSketch('sk4', function (p) {
  ...
  p.setup = function () {
    p.createCanvas(Math.min(p.windowWidth, 800), Math.min(p.windowHeight, 800));
  ...
  p.keyPressed = function () {
    ...
    if (p.key.toLowerCase() === 'e') pulse = !pulse;
  };
  ...
  p.windowResized = function () {
    p.resizeCanvas(Math.min(p.windowWidth, 800), Math.min(p.windowHeight, 800));
    recalc();
  };
  ...
  function drawDial(t, effort) {
    const rOuter = R * 1.05;
    p.noFill();
    p.stroke(40, 50, 60, 160); p.strokeWeight(12); p.circle(0, 0, R * 2.2);
    p.stroke(255, 230); p.strokeWeight(2); p.circle(0, 0, R * 2);

    const arcProg = p.map(t.s % 60, 0, 60, 0, p.TWO_PI);
    p.stroke(0, 0); p.noFill();
    p.push();
    p.rotate(-p.HALF_PI);
    p.stroke(30, 40, 60, 120); p.strokeWeight(8);
    p.arc(0, 0, rOuter * 2, rOuter * 2, 0, p.TWO_PI);
    const c1 = p.color(255, 210, 120, 220);
    const c2 = p.color(255, 140, 120, 220);
    p.stroke(p.lerpColor(c1, c2, p.constrain(effort,0,1))); p.strokeWeight(8);
    p.arc(0, 0, rOuter * 2, rOuter * 2, 0, arcProg);
    p.pop();
    ...
    const idx = Math.floor(p.constrain(effort, 0, 0.999) * 12);
    const rEm = R * 0.72;
    for (let i = 0; i < 12; i++) {
      const a = (i / 12) * p.TWO_PI - p.HALF_PI;
      const x = Math.cos(a) * rEm, y = Math.sin(a) * rEm;
      const base = Math.max(18, R * 0.11);
      const bump = pulse ? (0.06 * Math.sin(p.millis() / 600 + i)) : 0;
      const active = i === idx;
      p.textSize(base * (active ? 1.25 : 1.0) * (1 + bump));
      p.noStroke();
      p.fill(active ? 20 : 40);
      p.text(EMOJI[i], x, y);
      if (active) {
        p.noFill(); p.stroke(255, 220);
        p.strokeWeight(1.5);
        p.circle(x, y, base * 1.2);
      }
    }
  }
  ...
  function drawHUD(t, currSpeed, fat, eff) {
    const bw = 260, bh = 86, bx = 16, by = 16;
    ...
    p.text(`G=trail  R=reset  E=pulse`, bx + 10, by + 60);
  }
});